...
def profile_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, f'dear {new} your profile updated generated the following errors: {form.errors}')
...
def password_update(request):
    ...
    if request.method == 'POST':
        ...
        if passupdate.is_valid():
            ...
            messages.error(request, f'dear {new} a problem occured with the following errors: {passupdate.errors}')
...
def payment(request):
    if request.method == 'POST':
        ...
        api_key = 'sk_test_14f68aa9edd0d92c6ec54f52ef7f560296e6908d' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://13.50.248.108/callback' #payment confirmation page
